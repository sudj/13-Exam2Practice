...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        self.start_contents = self.contents
        self.history = []
    ...
    def append_string(self, additional_contents):
        ...
        if len(self.contents) > self.volume:
            for k in range(len(self.contents)-self.volume):
                self.additional_words = self.additional_words + self.contents[self.volume + k]
            for k in range(self.volume):
                word = word + self.contents[k]
            self.contents = word
        else:
            self.additional_words = ''
        return self.additional_words
    ...
    def double_then_shrink(self, new_volume):
        ...
        if (len(self.contents)*2) > self.volume:
            self.discard = self.discard + ((len(self.contents)*2) - self.volume)
        self.double()
        if len(self.contents) > new_volume:
            self.discard = self.discard + (len(self.contents) - new_volume)
    ...
    def steal(self, other_box):
        ...
        #    ** TWO **   LINES OF CODE.
        #######################################################################
        remain = self.append_string(other_box.contents)
        other_box.contents = remain
    ...
    def get_history(self):
        ...
        if len(self.history) is 0:
            ...
            #     history = []
            #     history = history + [self.history[k]]
            #     return
            return self.history
    ...
    def combined_box(self, other_box):
        ...
        together = Box(new_contents, new_volume)
        return together
